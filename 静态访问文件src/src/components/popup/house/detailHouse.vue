    ...
    <div data-in="pulse" class="detail-house-img animated fast-time" :class="{show: isShowOpacity}">
      <swiper :options="swiperOption" ref="city" class="swiper-content">
        <swiper-slide v-for="(img, imgIndex) in house.imgs" :key="imgIndex">
          <div class="img-content" :style="{backgroundImage: `url(${require('../../../static/data/imgs/'+img)})`}"></div>
        </swiper-slide>
...
<script>
  ...
  import oneHotel from '../../../static/data/one_hotel'
  ...
  export default {
    ...
    methods: {
      ...
      async getData() {
        ...
        this.house = oneHotel.data.hotel_dict[0];
        this.around_list = oneHotel.data.around_list;
        this.hotel_user = oneHotel.data.hotel_user;
        this.similar_hotel = oneHotel.data.similar_hotel;
        this.hotel_Appraise = oneHotel.data.hotel_Appraise;
      },
      scroll() {
