<template>
  <div class="collection">
    ...
    <div class="collection-stay" v-if="citys.length !== 0">
      ...
      <swiper :options="swiperOption" ref="city" class="swiper-content">
        <swiper-slide v-for="(item, index) in citys" :key="index">
          <div @click="handleHouseCollection(item)" class="img-content" :style="{backgroundImage: `url(${require('../static/data/imgs/'+item.hotel[0].imgs[0])})`}">
          </div>
    ...
    <div class="collection-food" v-if="food.length !== 0">
      <h1>美食</h1>
      <img :src="require('../static/data/imgs/'+food[0].belong_info[0].imgs[0])" alt="not find img" @click="handleFoodCollection">
      <p class="sub">{{food.length}} 种美食</p>
    </div>
    <div class="collection-story" v-if="story.length !== 0">
      <h1>故事</h1>
      <img :src="require('../static/data/imgs/'+story[0].belong_info[0].imgs[0])" alt="not find img" @click="handleStoryCollection">
...
<script>
  ...
  import collect from '../static/data/collect'
